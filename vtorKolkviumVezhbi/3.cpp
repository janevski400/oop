...
public:
    ...
    virtual int getBodovi ()
    {
        ...
        return tempBrojac/(float)brojOcenki*100;
    }
    virtual void pecati ()
    {
        cout<<indeks;
...
{
private:
    int brojChasoviNedela;
public:
    Demonstrator (int indeks=0, int*ocenki=nullptr, int brOcenki=0, char *ime=nullptr, Kurs *listaKursevi=nullptr, int brKurseviLab=0, int brojChasoviNedela=0): Student(indeks,ocenki,brOcenki), Predavach(ime,listaKursevi, brKurseviLab)
    {
        this->brojChasoviNedela = brojChasoviNedela;
    ...
    int getBodovi ()
    {
        if(getBrojKursevi()==0)
            ...
            return Student::getBodovi()+(20*brojChasoviNedela)/(float)getBrojKursevi();
    }
    void pecati ()
    {
    ...
    const char * getCourse (int i){
        return (this->operator[](i)).getIme();
    }
...
{
    int temp = studenti[0]->getBodovi();
    int in;
    for(int i=1;i<n;i++) {
        try {
            studenti[i]->getBodovi();
            if(studenti[i]->getBodovi()>temp) {
                temp=studenti[i]->getBodovi();
                in = i;
            }
        }
        catch (const char *) {
            cout<<"Demonstratorot so indeks ";
            studenti[i]->Student::pecati();
            cout<<" ne drzi laboratoriski vezbi"<<endl;
        }
    }
    return *studenti[in];
}
...
void pecatiDemonstratoriKurs (char* kurs, Student** studenti, int n) {
    for(int i=0;i<n;i++) {
        if(dynamic_cast<Demonstrator*>(studenti[i])) {
            ...
            if(!strcmp(temp.getCourse(i),kurs)) {
                temp.Demonstrator::pecati();
                cout<<endl;
            }
        }
    }
